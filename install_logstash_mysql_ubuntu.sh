...
exec 2> >(tee "./Logstash_Kibana3/install_logstash_mysql_ubuntu.err")
exec > >(tee "./Logstash_Kibana3/install_logstash_mysql_ubuntu.log")
...
logstash_conf="/etc/logstash/mysql_logstash.conf"
...
mkdir /etc/logstash
tee -a /etc/logstash/mysql_logstash.conf <<EOF
input {
  file {
