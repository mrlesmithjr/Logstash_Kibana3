...
exec 2> >(tee "./ELK-Processor-Node.err")
exec > >(tee "./ELK-Processor-Node.log")
...
# Install Pre-Reqs
apt-get install -y --force-yes git curl software-properties-common nginx ntp
...
echo "Installing Elasticsearch as a client node"
...
echo '#discovery.zen.ping.unicast.hosts: ["'$esnode1'", "'$esnode2'"]' >> /etc/elasticsearch/elasticsearch.yml
...
echo "Enter the HAProxy VIP (IP|HOSTNAME) for your logstash nodes"
echo "This will be what points to your back-end ELK Cluster Client Nodes"
echo "This will be the VIP, hostname or FQDN that was setup in haproxy.cfg"
